...
def run_graph(wav_data, labels, input_layer_name, output_layer_name,
  ...
  with tf.Session() as sess:
    ...
    for node_id in top_k:
      ...
      if score >= .7 and human_string == kwd:
          print(' kwd matched ')
...
def label_wav(wav, labels, graph, input_name, output_name, how_many_labels, kwd):
  ...
  if not kwd:
    tf.logging.fatal(' KeyWord not exist %s', kwd)
  ...
  run_graph(wav_data, labels_list, input_name, output_name, how_many_labels, kwd)


def main(_):
  """Entry point for script, converts flags to arguments."""
  label_wav(FLAGS.wav, FLAGS.labels, FLAGS.graph, FLAGS.input_name,
            FLAGS.output_name, FLAGS.how_many_labels, FLAGS.kwd)
...
if __name__ == '__main__':
  ...
  parser.add_argument(
      '--kwd', type=str, default='', help='keyword.') 
  parser.add_argument(
      '--how_many_labels',
      type=int,
      default=3,
